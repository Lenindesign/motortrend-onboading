 ...
 * Standalone navigation component for MotorTrend applications
 * No external CSS dependency - uses inline React styles
 */
...
/**
 * Search result type for vehicle search functionality
 */
...
/**
 * Filter options returned by getFilterOptions
 */
...
/**
 * Vehicle type for search and display
 */
...
export interface GlobalHeaderProps {
  ...
  onSignInClick?: () => void;
  /**
   * Called when profile menu item is clicked
   */
  onProfileClick?: () => void;
  /**
  ...
  /**
   * Custom logo URL (defaults to MotorTrend logo)
   */
  logoUrl?: string;
  /**
   * Logo click handler (defaults to navigating to '/')
   */
  onLogoClick?: () => void;
  /**
   * Search vehicles function - if not provided, search is disabled
   */
  searchVehicles?: (query: string, limit?: number) => SearchResult[];
  /**
   * Get filter options function - if not provided, research forms are disabled
   */
  getFilterOptions?: () => FilterOptions;
  /**
   * Get vehicles function for filtering by make
   */
  getVehicles?: (options: { make?: string[] }) => Vehicle[];
}

// Default no-op implementations
const defaultSearchVehicles = (): SearchResult[] => [];
const defaultGetFilterOptions = (): FilterOptions => ({ makes: [], years: [], bodyStyles: [] });
const defaultGetVehicles = (): Vehicle[] => [];

// Helper function to parse vehicle name (moved from utils)
function parseVehicleName(vehicleName: string): { year: string; make: string; model: string } | null {
  const match = vehicleName.match(/^(\d{4})\s+(\w+)\s+(.+)$/);
  if (match) {
...
export const GlobalHeader: React.FC<GlobalHeaderProps> = ({
  ...
  logoUrl = motorTrendLogo,
  searchVehicles = defaultSearchVehicles,
  getFilterOptions = defaultGetFilterOptions,
  getVehicles = defaultGetVehicles,
}) => {
