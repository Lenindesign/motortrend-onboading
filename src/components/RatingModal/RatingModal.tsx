...
export const RatingModal: React.FC<RatingModalProps> = ({
  ...
  const tooltipStyle: React.CSSProperties = { position: 'absolute', bottom: '100%', left: '50%', transform: 'translateX(-50%)', background: 'var(--color-white, #FFFFFF)', color: 'var(--color-neutrals-2)', padding: '8px 12px', borderRadius: 'var(--border-radius-md, 8px)', fontFamily: 'var(--font-body)', fontWeight: 600, fontSize: '13px', lineHeight: 1.3, whiteSpace: 'nowrap', boxShadow: '0 8px 20px rgba(0,0,0,0.25)', marginBottom: '16px', zIndex: 1001, animation: 'ratingTooltipFadeIn 0.2s ease-out forwards' };
  ...
  const btnBaseStyle: React.CSSProperties = { display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '16px 32px', borderRadius: 'var(--border-radius-md-lg, 12px)', fontFamily: 'var(--font-heading)', fontWeight: 600, fontSize: '14px', lineHeight: 1, cursor: 'pointer', transition: 'all 0.2s cubic-bezier(0.4,0,0.2,1)', border: 'none', textTransform: 'uppercase', letterSpacing: '1px', flex: 1, maxWidth: '200px' };
  const submitBtnStyle: React.CSSProperties = { ...btnBaseStyle, background: isSubmitHovered && selectedRating !== 0 ? 'rgba(255,255,255,0.2)' : 'rgba(255,255,255,0.1)', color: 'var(--color-white, #FFFFFF)', border: '1px solid rgba(255,255,255,0.1)', transform: isSubmitHovered && selectedRating !== 0 ? 'translateY(-2px)' : 'none', opacity: selectedRating === 0 && currentRating === 0 ? 0.5 : 1 };
