...
export const AvatarBannerModal: React.FC<AvatarBannerModalProps> = ({
  ...
  const closeBtnStyle: React.CSSProperties = { display: 'flex', alignItems: 'center', justifyContent: 'center', width: '32px', height: '32px', background: isCloseHovered ? 'var(--color-neutrals-7, #F4F5F6)' : 'none', border: 'none', borderRadius: '50%', color: isCloseHovered ? 'var(--color-neutrals-2, #23262F)' : 'var(--color-neutrals-4, #6E7481)', cursor: 'pointer', transition: 'all 150ms ease-in-out' };
  ...
  const getOptionStyle = (optionId: string, isSelected: boolean, isBanner: boolean): React.CSSProperties => {
    ...
    return { position: 'relative', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '8px', padding: isBanner ? '8px' : '8px', border: `2px solid ${isSelected ? 'var(--color-neutrals-2, #23262F)' : (isHovered ? 'var(--color-neutrals-5, #B1B5C3)' : 'var(--color-neutrals-6, #E6E8EC)')}`, borderRadius: '8px', cursor: 'pointer', backgroundColor: isSelected ? 'var(--color-neutrals-7, #F4F5F6)' : 'var(--color-white, #FFFFFF)', boxShadow: isSelected || isHovered ? '0 4px 20px rgba(20, 20, 22, 0.06)' : '0 1px 2px rgba(20, 20, 22, 0.04)', transform: isSelected || isHovered ? 'translateY(-2px)' : 'none', transition: 'all 150ms ease-in-out' };
  };

  const getImageContainerStyle = (isLogo: boolean, isBanner: boolean): React.CSSProperties => ({ width: isBanner ? '100%' : '80px', height: isBanner ? '100px' : '80px', borderRadius: isBanner ? '4px' : '50%', overflow: 'hidden', backgroundColor: 'var(--color-neutrals-7, #F4F5F6)', display: 'flex', alignItems: 'center', justifyContent: 'center' });
  ...
  const checkStyle: React.CSSProperties = { position: 'absolute', top: '4px', right: '4px', width: '24px', height: '24px', backgroundColor: 'var(--color-neutrals-2, #23262F)', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--color-neutrals-8, #FCFCFD)', boxShadow: '0 2px 8px rgba(20, 20, 22, 0.04)' };
