 ...
 * Index page displaying video content and related articles
 */
...
const Videos: React.FC = () => {
  ...
  // Get all articles sorted by date (newest first) - these could have video content
  const videoArticles = useMemo(() => {
    return Object.entries(articles)
      ...
      .slice(0, 20); // Limit to 20 most recent for video content
  }, []);

  // Get newest vehicles (2025 and 2026 models) - these might have video walkarounds
  const newestVehicles = useMemo(() => {
    return carDatabase
      .filter(vehicleName => {
        const yearMatch = vehicleName.match(/^(2025|2026)\s/);
  ...
  // Get unique newest vehicles (by make/model)
  const uniqueNewestVehicles = useMemo(() => {
  ...
  // Hero data - use first video article
  const heroData = useMemo(() => {
    if (videoArticles.length > 0) {
      const firstArticle = videoArticles[0];
      return {
    ...
    return {
      ...
      title: 'Videos: Watch the Latest Automotive Content',
      onClick: () => navigate('/videos'),
    };
  }, [videoArticles, navigate]);

  // Vertical cards - next 3 video articles
  const verticalCards = useMemo(() => {
    return videoArticles.slice(1, 4).map(({ slug, article }) => ({
  ...
  // Vehicles for VehiclesSection - newest vehicles (might have video walkarounds)
  const vehiclesData = useMemo(() => {
    return uniqueNewestVehicles.slice(0, 12).map(vehicle => ({
      name: vehicle
    }));
  }, [uniqueNewestVehicles]);

  // Video items for river section - remaining video articles
  const videoItems = useMemo(() => {
    return videoArticles.slice(4).map(({ slug, article }) => ({
      imageUrl: article.heroImage,
      title: article.title,
    ...
    <div className="videos">
      <div className="videos__container">
        ...
        <div className="videos__section">
          <div className="videos__left-column">
            <HeroPlusThree
              hero={heroData}
              cards={verticalCards}
            />
          </div>
          <div className="videos__right-column">
            <AdContainer
              width={300}
              height={250}
              label="300 x 250"
        ...
        {/* Vehicles Section - Vehicles with Video Content */}
        {vehiclesData.length > 0 && (
          <div className="videos__section">
            <div className="videos__left-column">
              <VehiclesSection
                title="Vehicle Walkarounds & Reviews"
                vehicles={vehiclesData}
                showMoreVisible={uniqueNewestVehicles.length > 12}
                onShowMore={() => {
                  navigate('/new-cars');
                }}
              />
